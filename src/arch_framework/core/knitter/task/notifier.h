...
//   ec.notify(true);
//
// notify is cheap if there are no waiting threads. prepare_wait/commit_wait are
// not
...
public:
    ...
    // and call either cancel_wait or commit_wait passing the same Waiter
    // object.
    ...
    void commit_wait(Waiter* w) {
        ...
        for (;;) {
            if (int64_t((state & kEpochMask) - epoch) < 0) {
                // The preceeding waiter has not decided on its fate. Wait until
                // it
            ...
            if (int64_t((state & kEpochMask) - epoch) > 0)
                return;
            // Remove this thread from prewait counter and add it to the waiter
            // list.
    ...
    void notify(bool all) {
        ...
        for (;;) {
            ...
            if (all) {
                ...
                if (wnext != nullptr)
                    next = wnext - &_waiters[0];
                // Note: we don't add kEpochInc here. ABA problem on the
                // lock-free stack
                // can't happen because a waiter is re-pushed onto the stack
                // only after
...
private:
    ...
    Notifier(Notifier&& rhs) : _state{rhs._state.load()}, _waiters{std::move(rhs._waiters)} {
    }
};
