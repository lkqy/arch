...
class PassiveVector {
    static_assert(std::is_trivial<T>::value && std::is_standard_layout<T>::value, "must be a plain old data type");
...
public:
    ...
    PassiveVector() noexcept : _data{reinterpret_cast<pointer>(_stack)}, _num{0}, _cap{S} {
    }
    ...
    bool in_stack() const {
        return _data == reinterpret_cast<const_pointer>(_stack);
    }
