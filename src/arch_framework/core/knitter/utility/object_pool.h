// 2020/06/27 - modified by Sumory Wu
...
template <typename T>
constexpr T const& trick_max_to_avoid_gcc4_constexpr_unsupport(T const& a, T const& b) {
    return a > b ? a : b;
}

// FIXME:
// Mind: gcc4 & gcc8 ObjectPool<Node> different size?
...
public:
    ...
    static const size_t X = trick_max_to_avoid_gcc4_constexpr_unsupport(sizeof(T*), sizeof(T));

    // constexpr static size_t X = sizeof(long double) + std::max(sizeof(T*),
    // sizeof(T));
...
//
template <typename T, size_t S>
void ObjectPool<T, S>::_blocklist_push_front(Blocklist* curr, Blocklist* head) {
...
template <typename T, size_t S>
void ObjectPool<T, S>::_blocklist_replace(Blocklist* old, Blocklist* curr) {
    curr->next = old->next;
    curr->next->prev = curr;
    curr->prev = old->prev;
    curr->prev->next = curr;
...
template <typename T, size_t S>
void ObjectPool<T, S>::_blocklist_move_front(Blocklist* list, Blocklist* head) {
...
template <typename T, size_t S>
void ObjectPool<T, S>::_blocklist_move_back(Blocklist* list, Blocklist* head) {
    _blocklist_del_impl(list->prev, list->next);
...
template <typename T, size_t S>
bool ObjectPool<T, S>::_blocklist_is_first(const Blocklist* list, const Blocklist* head) {
...
// @head: the head of the list
template <typename T, size_t S>
bool ObjectPool<T, S>::_blocklist_is_last(const Blocklist* list, const Blocklist* head) {
                ...
                if ((h->u + K * M < h->a) && (h->u < ((F - 1) * h->a / F))) {
                    for (size_t i = 0; i < F; i++) {
                        if (!_blocklist_is_empty(&h->lists[i])) {
                            ...
                            // printf("transfer a block (x.u=%lu/x.i=%lu) to the
                            // global heap\n", x->u, x->i);
