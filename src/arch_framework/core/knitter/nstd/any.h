...
#define nstd_lite_in_place(T) \
    std::in_place_t {         \
    }
#define nstd_lite_in_place_type(T) \
    std::in_place_type_t<T> {      \
    }
#define nstd_lite_in_place_index(K) \
...
// Compiler versions:
//
// MSVC++  6.0  _MSC_VER == 1200  any_COMPILER_MSVC_VERSION ==  60  (Visual
// Studio 6.0)
// MSVC++  7.0  _MSC_VER == 1300  any_COMPILER_MSVC_VERSION ==  70  (Visual
// Studio .NET 2002)
// MSVC++  7.1  _MSC_VER == 1310  any_COMPILER_MSVC_VERSION ==  71  (Visual
// Studio .NET 2003)
// MSVC++  8.0  _MSC_VER == 1400  any_COMPILER_MSVC_VERSION ==  80  (Visual
// Studio 2005)
// MSVC++  9.0  _MSC_VER == 1500  any_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008)
// MSVC++ 10.0  _MSC_VER == 1600  any_COMPILER_MSVC_VERSION == 100  (Visual
// Studio 2010)
// MSVC++ 11.0  _MSC_VER == 1700  any_COMPILER_MSVC_VERSION == 110  (Visual
// Studio 2012)
// MSVC++ 12.0  _MSC_VER == 1800  any_COMPILER_MSVC_VERSION == 120  (Visual
// Studio 2013)
// MSVC++ 14.0  _MSC_VER == 1900  any_COMPILER_MSVC_VERSION == 140  (Visual
// Studio 2015)
// MSVC++ 14.1  _MSC_VER >= 1910  any_COMPILER_MSVC_VERSION == 141  (Visual
// Studio 2017)
// MSVC++ 14.2  _MSC_VER >= 1920  any_COMPILER_MSVC_VERSION == 142  (Visual
// Studio 2019)
...
#define any_REQUIRES_0(...) template <bool B = (__VA_ARGS__), typename std::enable_if<B, int>::type = 0>

#define any_REQUIRES_T(...) , typename = typename std::enable_if<(__VA_ARGS__), nstd::any_lite::detail::enabler>::type
...
public:
    ...
    template <class ValueType,
              class T = typename std::decay<ValueType>::type any_REQUIRES_T(!std::is_same<T, any>::value)>
    any(ValueType&& value) any_noexcept : content(new holder<T>(std::forward<ValueType>(value))) {
    }
    ...
    template <class T, class U,
              class... Args any_REQUIRES_T(std::is_constructible<T, std::initializer_list<U>&, Args&&...>::value)>
...
template <class ValueType
          ...
          typename = typename std::enable_if<(std::is_reference<ValueType>::value ||
                                             ...
                                             nstd::any_lite::detail::enabler>::type
#endif
          >
any_nodiscard inline ValueType any_cast(any const& operand) {
    const ValueType* result =
            any_cast<typename std11::add_const<typename std11::remove_reference<ValueType>::type>::type>(&operand);
