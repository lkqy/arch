...
// Studio 6.0)
// MSVC++  7.0  _MSC_VER == 1300  optional_COMPILER_MSVC_VERSION ==  70  (Visual
// Studio .NET 2002)
// MSVC++  7.1  _MSC_VER == 1310  optional_COMPILER_MSVC_VERSION ==  71  (Visual
// Studio .NET 2003)
// MSVC++  8.0  _MSC_VER == 1400  optional_COMPILER_MSVC_VERSION ==  80  (Visual
// Studio 2005)
// MSVC++  9.0  _MSC_VER == 1500  optional_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008)
// MSVC++ 10.0  _MSC_VER == 1600  optional_COMPILER_MSVC_VERSION == 100  (Visual
// Studio 2010)
// MSVC++ 11.0  _MSC_VER == 1700  optional_COMPILER_MSVC_VERSION == 110  (Visual
// Studio 2012)
// MSVC++ 12.0  _MSC_VER == 1800  optional_COMPILER_MSVC_VERSION == 120  (Visual
// Studio 2013)
// MSVC++ 14.0  _MSC_VER == 1900  optional_COMPILER_MSVC_VERSION == 140  (Visual
// Studio 2015)
// MSVC++ 14.1  _MSC_VER >= 1910  optional_COMPILER_MSVC_VERSION == 141  (Visual
// Studio 2017)
// MSVC++ 14.2  _MSC_VER >= 1920  optional_COMPILER_MSVC_VERSION == 142  (Visual
// Studio 2019)
...
struct is_nothrow_swappable {
    // wrap noexcept(expr) in separate function as work-around for VC140
    // (VS2015):
...
// >
    ...
    // 4b (C++98 and later) - non-explicit converting copy-construct from
    // optional
    template <typename U
#if optional_CPP11_OR_GREATER
    ...
    template <typename U optional_REQUIRES_T(
            ...
            !std::is_constructible<T, optional<U> const&&>::value && !std::is_convertible<optional<U>&, T>::value &&
            !std::is_convertible<optional<U>&&, T>::value && !std::is_convertible<optional<U> const&, T>::value &&
            !std::is_convertible<optional<U> const&&, T>::value && !std::is_convertible<U&&, T>::value /*=> explicit */
            )>
    ...
    optional_constexpr explicit optional(U&& value) : has_value_(true), contained(T{std::forward<U>(value)}) {
    }
    ...
    // NOLINTNEXTLINE( google-explicit-constructor, hicpp-explicit-conversions )
    optional_constexpr /*non-explicit*/ optional(U&& value) : has_value_(true), contained(std::forward<U>(value)) {
    }
